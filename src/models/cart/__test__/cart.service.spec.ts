...
describe('CartService', () => {
      ...
      providers: [
        CartService,
        {
          ...
          useValue: {
            ...
            updatedProductInCart: jest.fn().mockResolvedValue(undefined),
          },
        },
        {
  ...
  it('should return cart in updatedProductInCart', async () => {
    const spyCartProductService = jest.spyOn(
      cartProductService,
      'updatedProductInCart',
    );
    const spySave = jest.spyOn(cartRepository, 'save');
    const cart = await service.updatedProductInCart(
      updateCartMock,
      userEntityMock.id,
    );
